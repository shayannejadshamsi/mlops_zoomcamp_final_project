...
EXPERIMENT_NAME = "customer-churn-project"
...
def performance_metrics(X_val, y_val, y_pred, session, logreg):
    ...
    with session.logger(tags={"datasetId": "model-4"}, dataset_timestamp=datetime.now()) as ylog:
        ylog.log_metrics(
            targets=list(y_val),
...
def starting_whylogs():
    ...
    try:
        os.environ["WHYLABS_API_KEY"] = "LiqEneirxJ.ub4oSRZJOVUqk2Grg3TI9GXikSp4MhMCzCNTFKB4eahIywc8jFvlW:org-jk62GD"
    except Exception as e:
        print("WHYLABS_API_KEY not found" + str(e))
        #os.environ["WHYLABS_API_KEY"] = os.environ["WHYLOGS_ACCOUNT_KEY"]
        raise
    os.environ["WHYLABS_DEFAULT_ORG_ID"] = "org-jk62GD"
    # Adding the WhyLabs Writer to utilize WhyLabs platform
    writer = WhyLabsWriter("", formats=[])
    session = Session(project="model-4", pipeline="mlops-project-pipeline", writers=[writer])
...
def model_performance_dashboard(df_train, train_dicts, df_val, val_dicts, numerical_features, categorical_features):
    ...
    df_train.rename(columns={"Churn": "target"}, inplace=True)
    df_val.rename(columns={"Churn": "target"}, inplace=True)
    ...
    # Save dashboard
    df_model_performance_dashboard.save("dashboards/performance_output.html")
...
def transform_data(df: pd.DataFrame):
    ...
    ## Training model
    df_train, df_val = train_test_split(df_train_all, test_size=0.2, random_state=0)
...
def applying_model():
    ...
    with mlflow.start_run():
        ...
        mlflow.set_tag("developer", "Shayan")
        mlflow.log_param("train-data-path", "datasets/customer_data.csv")
        mlflow.log_param("val-data-path", "datasets/customer_data.csv")
        # Create Model
